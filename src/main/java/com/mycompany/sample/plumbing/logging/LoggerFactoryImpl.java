...
public final class LoggerFactoryImpl implements LoggerFactory {
    ...
    private ArrayList<PerformanceThreshold> thresholdOverrides;
    ...
    public LoggerFactoryImpl() {
        ...
        final var defaultThresholdValue = 1000;
        this.defaultPerformanceThresholdMilliseconds = defaultThresholdValue;
    ...
    private ConsoleAppender<ILoggingEvent> createProductionConsoleAppender(
        ...
        // The log data is bare JSON without any logback fields, and uses pretty printing for readability
    ...
    private int getPerformanceThreshold(final String name) {
        ...
        var found = this.thresholdOverrides.stream().filter(
                p -> p.getName().toLowerCase().equals(name.toLowerCase())).findFirst();
